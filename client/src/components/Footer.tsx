...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="relative bg-gradient-to-b from-gray-900/95 to-black border-t border-primary-gold/30 overflow-hidden">
      {/* Subtle Background Effects */}
      <div className="absolute inset-0">
        <div className="absolute top-0 left-1/3 w-64 h-64 bg-primary-gold/8 rounded-full blur-2xl" />
        <div className="absolute bottom-0 right-1/3 w-64 h-64 bg-primary-orange/8 rounded-full blur-2xl" />
                ...
                <div className="flex space-x-4">
                  {socialLinks.map((social, index) => (
                    <a
                      key={index}
                      href={social.href}
                      className={`w-12 h-12 bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center text-white transition-all duration-300 hover:scale-105 ${social.color} hover:bg-white/20 group`}
              ...
              <ul className="space-y-3">
                {quickLinks.map((link, index) => (
                  <li key={index}>
                    <Link 
                      to={link.href}
                      className="text-enhanced hover:text-accent transition-colors duration-300 font-medium hover:translate-x-1 transform inline-block"
                    >
                      {link.label}
                    </Link>
                  </li>
            ...
            <div className="animate-fade-in-up delay-300">
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-3 group">
                  <div className="w-10 h-10 bg-gradient-to-r from-primary-gold to-primary-orange rounded-full flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="mt-8 p-4 bg-white/5 backdrop-blur-sm rounded-xl border border-primary-gold/20">
                <div className="flex items-center space-x-2 mb-2">
                  <div className="flex space-x-1">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} size={16} className="text-primary-gold fill-current" />
                    ))}
                  </div>
          ...
          <div className="border-t border-primary-gold/30 pt-8 mt-8">
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
              ...
              <div className="flex items-center space-x-2 text-bright text-base font-medium">
                <span>Made with</span>
                <Heart className="text-primary-gold fill-current" size={18} />
